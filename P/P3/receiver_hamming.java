...
class receiver_hamming
{
    public static void main(String args[]) throws Exception
    {
        try{
            ...
            System.out.println("server listening on localhost:6363");
            ...
            int rhcode[] = new int[totalbits+1];
            ...
            //create array to store errorbits
            int errorindex[] = new int[rbits];
            ...
            for(int i=1,x=0,e=0; i<=totalbits; i++)
            {
                if(Math.pow(2,x)==i)
                {
                    ...
                    for(int sindex=i; sindex<=totalbits; sindex=sindex+i+i)
                    {
                            ...
                            {
                                    //System.out.println("p["+index+"] : "+hcode[index]);
                                    if(rhcode[index] == 1)
                                    {
                                        counter++;
                                    }
            ...
            //if sum < 0 then no error else error
            if(sum>0)
            {
                System.out.println("\n Error at index: "+ sum);
                //correct error, flip the bit at error index (0 to 1) & (1 to 0)
                if(rhcode[sum]==0)
                    rhcode[sum]=1;
                else 
                    rhcode[sum]=0;
            ...
            //extarct data from hamming code
            int rdata[] = new int[databits+1];
            for(int i=1,x=1,d=0; i<=totalbits; i++)
            {
                if(Math.pow(2,x)==i)
                {
                    x++;
