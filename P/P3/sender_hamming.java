...
class sender_hamming
{
    public static void main(String args[]) throws Exception
    {
        try{
            ...
            System.out.print("Enter Length of data:");
            int databits = sc.nextInt();

            int rbits = 0;
            System.out.println("\nCalculating rbits:");
            while(Math.pow(2,rbits) < databits+rbits+1)
            {
                System.out.println("2^" + rbits + " < " +databits+" + "+rbits+" + 1");
                rbits++;
            }   

            int totalbits = databits + rbits;
            System.out.println("\nData bits: "+databits);
            System.out.println("Redundent Bits: "+rbits);
            ...
            //create array & store hamming code
            int hcode[] = new int[totalbits+1];
            ...
            //calculating value of rbits
            for(int i=1,x=0; i<=totalbits;i++)
            {
                if(Math.pow(2,x)==i)
                {
                    int counter = 0;
                    ...
                    for(int sindex=i; sindex<=totalbits; sindex=sindex+i+i)
                    {
                            ...
                            {
                                    System.out.print("\tp["+index+"] : "+hcode[index]);
                                    if(hcode[index] == 1)
                                    {
                                        counter++;
                                    }
            ...
            DatagramPacket dp1 = new DatagramPacket(store.getBytes(),store.length(),ip,6363);
            ds1.send(dp1);
